...
mkdir -p "$ISAAC_ROOT/$OUTPUT_DIR"
cd "$ISAAC_ROOT"

SCREEN_LIST="$(screen -ls 2>/dev/null || true)"

printf 'Starting Franka collection shards\n'
printf '  total_envs=%s episodes=%s gpus=%s shards_per_gpu=%s total_shards=%s\n' \
    "$TOTAL_ENVS" "$EPISODES" "$GPUS" "$SHARDS_PER_GPU" "$TOTAL_SHARDS"
...
for gpu in "${GPU_IDS[@]}"; do
    for ((local_shard = 0; local_shard < SHARDS_PER_GPU; local_shard++)); do
        ...
        if ((DRY_RUN == 0)); then
            screen -dmS "$session_name" bash -lc "cd '$ISAAC_ROOT' && $(printf '%q ' "${cmd[@]}")> '$log_path' 2>&1"
...
if ((DRY_RUN)); then
    ...
    printf '  scripts/franka_collection_progress.sh --run-prefix %q --session-prefix %q\n' "$RUN_PREFIX" "$SESSION_PREFIX"
fi
