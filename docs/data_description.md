...
- 当前仓库标准只接受这 20 个有效通道；非标准压力 CSV 应在进入主线流程前重新生成。
...
`pressure/pressure.csv` 必须包含 `sensor_timestamp_us`、`host_monotonic_us` 和标准 20 个 `CH*` 列。通道列会按 `channel_mapping.json` 中的有效通道顺序进入模型。
