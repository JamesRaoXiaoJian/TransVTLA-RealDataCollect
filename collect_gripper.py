...
def compose_preview(
    ...
    cv2.putText(info_panel, gripper_text, (20, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (220, 220, 220), 2, cv2.LINE_AA)
    ...
    draw_pressure_dashboard(info_panel, 20, 176, top_width - 40, pressure_values)
...
def main() -> None:
    ...
    gripper: Optional[GripperStateCollector] = None
    ...
    latest_gripper_text = "Gripper: N/A"
    last_visual_time = 0.0

    print("Press SPACE to start/stop recording sessions, Q/ESC to exit.")
    print(
        f"Sampling rates: Visual={VISUAL_FPS}Hz, Tactile={TACTILE_FPS}Hz, "
        f"Robot Arm={ROBOT_ARM_FPS}Hz, Gripper target={GRIPPER_FPS}Hz"
    )
        ...
        while True:
            ...
            if (now - last_visual_time) < VISUAL_INTERVAL_S:
                ...
                if key == ord(" "):
                    ...
                    if recording:
                        ...
                        if gripper is not None:
                            gripper.start_session(session_paths.root)
                        frame_id = 0
            ...
            if gripper is not None:
                ...
                if gripper_pos is None:
                    ...
                    latest_gripper_text = (
                        f"Gripper: code={gripper_code} pos={gripper_pos} "
                        f"read={gripper_latency:.1f}ms"
                    )
            else:
                latest_gripper_text = "Gripper: disabled"
